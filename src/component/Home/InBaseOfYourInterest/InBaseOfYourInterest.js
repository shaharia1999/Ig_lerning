...
function InBaseOfYourInterest() {
    const [InBasedOfYourInterestedData, setInBasedOfYourInterestedData] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [BasedTotal, setBasedTotal] = useState(0);

    useEffect(() => {
        setIsLoading(true);
        axios.get(`${ApiUrl.BaseUrl}api/course/in-based-on-your-interested-course/`).then((response) => {
            if (response.data.error === false) {
                setBasedTotal(response.data.total_count);
                setInBasedOfYourInterestedData(response.data.data);
                setIsLoading(false);
            }
        });
    }, []);
    ...
    const InBaseOfYourInterestLoading = (() => {
        return (
            <>
    ...
    const InBaseOfYourInterest_Data = (() => {
        return (
            InBasedOfYourInterestedData.map((intellogeek_data) => (
        ...
        return (
            <Fragment>
                <div className="container 2xl:my-12 xl:my-12 lg:my-12 my-12 2xl:px-16 xl:px-12 lg:px-16 md:px-8 sm:px-8">
                    <h4 className="2xl:text-3xl xl:text-2xl lg:text-xl text-xl font-semibold text-sectionTitleColor dark:text-white 2xl:ml-6 xl:ml-4 lg:ml-3 ml-4 -mt-6">In Base Of Your Interest ({BasedTotal}) </h4>
...
export default InBaseOfYourInterest;