...
function RecentlyAddedCourseSectionTwo() {
    ...
    function fetchData(category_id) {
        axios.get(ApiUrl.BaseUrl + 'api/course/course-subcategory-response/' + category_id + "/",
            {
                headers: {
                    'Accept-Language': 'bn',
                    'Content-Type': 'application/json',
    ...
    const category_data = category.map((category_info, index) => (
        <li className="nav-item" role="presentation">
               <button type="button" className="nav-link1 block w-full md:w-auto px-6 py-3 my-2 md:mr-2 active text-base font-medium leading-tight mx-2 h-11" id="pills-master-tab3" data-bs-toggle="pill" data-bs-target="#pills-master" role="tab" aria-controls="pills-master" aria-selected="true">{category_info.category_name}</button>
        </li>
    ));
