...
function RecentlyViewedCourse() {
    ...
    const [RecentlyViewedCourseData, setRecentlyViewedCourseData] = useState([]);
    const [isLoading, setIsLoading] = useState(false);
    const [BasedTotal, setBasedTotal] = useState(0);

    useEffect(() => {
        setIsLoading(true);
        axios.get(`${ApiUrl.BaseUrl}api/course/recently-viewed-product/`).then((response) => {
            if (response.data.error === false) {
                setBasedTotal(response.data.total_count);
                setRecentlyViewedCourseData(response.data.data);
                setIsLoading(false);
            }
        });
    }, []);

    const RecentlyViewedCourseLoading = (() => {
        return (
            <>
    ...
    const RecentlyViewedCourseSuccess_Data = (() => {
        return (
            RecentlyViewedCourseData.map((intellogeek_data) => (
                ...
                <div className="xl:ml-28 container">
                    <div className="mt-4">
                        <div className="xl:-mr-64">
                            ...
                            >
                                {
                                    RecentlyViewedCourseSuccess_Data
                                }
                            </Slider>
                        </div>
                    </div>
